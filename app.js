...
import { User, Client, Op } from './models/index.js';
...
// --- Auth ---
app.get('/register', authLimiter, (req, res) => {
...
app.post('/register', authLimiter, async (req, res) => {
  try {
    ...
    if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{8,}$/.test(password)) {
      req.flash(
        'error',
        'Mot de passe doit contenir au moins 8 caractères, maj, min, chiffre et symbole.'
      );
      return res.redirect('/register');
    }
...
// --- Appointments via routeur dédié ---
...
app.use((err, req, res, next) => {
  if (err && err.code === 'EBADCSRFTOKEN') {
    ...
    const sameOrigin =
      referrer && referrer.startsWith(`${req.protocol}://${req.get('host')}`);
